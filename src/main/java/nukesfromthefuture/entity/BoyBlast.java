...
public class BoyBlast extends Entity{

	public ExplosionMan	hoool = null;
	public double		radius;
	public int			time		= 0;
	
	public BoyBlast(World par1World)
	{
		super(par1World);
		ignoreFrustumCheck = true;
	}
	
	public BoyBlast(World par1World, float x, float y, float z, ExplosionMan hool, int rad)
	{
		super(par1World);
		ignoreFrustumCheck = true;
		hoool = hool;
		radius = rad;
		motionX = Math.sqrt(radius - Nukesfromthefuture.Boystrength) / 10;
		setPosition(x, y, z);
	}
	
	public BoyBlast(World par1World, double x, double y, double z, float rad)
	{
		super(par1World);
		ignoreFrustumCheck = true;
		radius = rad;
		motionX = Math.sqrt(rad - Nukesfromthefuture.Boystrength) / 10;
		setPosition(x, y, z);
	}
