...
public class Flood extends Entity {
	public FloodExplosion hoool = null;
	public double		radius;
	public int			time		= 0;
	
	public Flood(World par1World)
	{
		super(par1World);
		ignoreFrustumCheck = true;
	}
	
	public Flood(World par1World, float x, float y, float z, FloodExplosion hool, int rad)
	{
		super(par1World);
		ignoreFrustumCheck = true;
		hoool = hool;
		radius = rad;
		motionX = Math.sqrt(radius - Nukesfromthefuture.flood_strength) / 10;
		setPosition(x, y, z);
	}
	
	public Flood(World par1World, double x, double y, double z, float rad)
	{
		super(par1World);
		ignoreFrustumCheck = true;
		radius = rad;
		motionX = Math.sqrt(rad - Nukesfromthefuture.flood_strength) / 10;
		setPosition(x, y, z);
	}
