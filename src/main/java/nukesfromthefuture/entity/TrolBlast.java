...
public class TrolBlast extends Entity{
	public TrololoExplosion	hoool = null;
	public double		radius;
	public int			time		= 0;
	
	public TrolBlast(World par1World)
	{
		super(par1World);
		ignoreFrustumCheck = true;
	}
	
	public TrolBlast(World par1World, float x, float y, float z, TrololoExplosion hool, int rad)
	{
		super(par1World);
		ignoreFrustumCheck = true;
		hoool = hool;
		radius = rad;
		motionX = Math.sqrt(radius - Nukesfromthefuture.trol_strength) / 10;
		setPosition(x, y, z);
	}
	
	public TrolBlast(World par1World, double x, double y, double z, float rad)
	{
		super(par1World);
		ignoreFrustumCheck = true;
		radius = rad;
		motionX = Math.sqrt(rad - Nukesfromthefuture.trol_strength) / 10;
		setPosition(x, y, z);
	}
	...
	public void onUpdate()
	{
		...
		{
			worldObj.playSoundAtEntity(this, Reference.MOD_ID + ":" + "nukes.trol", 10.0F, 0.50F);
			if(rand.nextInt(5) == 0)
		...
		if (!worldObj.isRemote)
		{
			...
			for (int i = 0; i < Nukesfromthefuture.trol_speed * 2; i++)
			{
				if (hoool != null)
				{
					hoool.update(this);
