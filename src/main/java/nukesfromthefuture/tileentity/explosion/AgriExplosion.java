...
public class AgriExplosion {
	...
	public AgriExplosion(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(UwUEx lul)
	{
		...
		if (dist < radius * radius)
		{
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					if (Y == 0) break;
					Block block = worldObj.getBlock(x + posX, Y, z + posZ);
					if (blockType == 1) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.farmland);
					else if (blockType == 2) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.water);
					else if (blockType == 3) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.water);
					else worldObj.setBlock(x + posX, Y, z + posZ, Blocks.farmland);
			...
			if (isTree)
			{
				...
				{
					if (Y == 0) break;
					worldObj.setBlock(x + posX, Y, z + posZ, Blocks.flowing_water);
					worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
				}
			}
		...
		else if (dist <= radius * radius * 1.3125 * 1.3125)
		{
			...
			if (dist >= radius + 5)
			{
				...
				for (int Y = ylimit; Y >= 0; Y--)
				{
					...
					else
					{
						...
						{
							worldObj.setBlock(x + posX, yy + Yy, z + posZ, Blocks.flowing_water);
			...
			else
			{
				...
				if (isTree)
				{
					...
					for (int Y = ylimit; Y > ylimit - treeHeight; Y--)
					{
						worldObj.setBlock(x + posX, Y, z + posZ, Blocks.water);
						worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
					}
				}
			}
