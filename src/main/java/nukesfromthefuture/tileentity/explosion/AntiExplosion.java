...
public class AntiExplosion {
	...
	public AntiExplosion(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(TimeBlast lul)
	{
		...
		if (n > 0 && n < nlimit)
		{
			...
			{
				repeatCount++;
				if (repeatCount < Nukesfromthefuture.antitimespeed * 2) update(lul);
				else
				{
					repeatCount = 0;
					return true;
	...
	private boolean processChunk(int x, int z)
	{
		...
		if (dist < radius * radius)
		{
			...
			if (Nukesfromthefuture.elevation) yele = y;
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					if (Y == 0) break;
					Block block = worldObj.getBlock(x + posX, Y, z + posZ);
					if (blockType == 1) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.time_fissures);
					else if (blockType == 2) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.time_fissures);
					else if (blockType == 3) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.time_fissures);
					else worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.time_fissures);
			...
			if (isTree)
			{
				...
				{
					if (Y == 0) break;
					worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.time_fissure_log);
					worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
				}
			}
		...
		else if (dist <= radius * radius * 1.3125 * 1.3125)
		{
			...
			if (dist >= radius + 5)
			{
				...
				for (int Y = ylimit; Y >= 0; Y--)
				{
					...
					if (blockID == Nukesfromthefuture.big_boy);
					else if (blockID == Nukesfromthefuture.skinny_man);
					else if (!isTree)
					{
					...
					else
					{
						...
						{
							worldObj.setBlock(x + posX, yy + Yy, z + posZ, Nukesfromthefuture.time_fissure_log);
			...
			else
			{
				...
				if (isTree)
				{
					...
					for (int Y = ylimit; Y > ylimit - treeHeight; Y--)
					{
						worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.time_fissures);
						worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
					}
				}
			}
	...
	private int getTopBlock(int x, int z, double dist)
	{
		...
		for (int y = 256; y > 0; y--)
		{
			...
			if (blockID != Blocks.air)
			{
				...
				if (blockID == Nukesfromthefuture.unrefinary)
				{
