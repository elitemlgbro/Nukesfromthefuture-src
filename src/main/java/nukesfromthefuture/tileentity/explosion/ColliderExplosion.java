...
public class ColliderExplosion
{
	public int		posX;
	public int		posY;
	public int		posZ;
	...
	public ColliderExplosion(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(EntityColliderBlast tsarblast)
	{
		...
		if (n > 0 && n < nlimit)
		{
			...
			{
				repeatCount++;
				if (repeatCount < Nukesfromthefuture.colliderSpeed* 2) update(tsarblast);
				else
				{
					repeatCount = 0;
					return true;
		...
		else if (dist <= radius * radius * 1.3125 * 1.3125)
		{
			...
			else
			{
				...
				if (isTree)
				{
					...
					for (int Y = ylimit; Y > ylimit - treeHeight; Y--)
					{
						worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.ego_ore);
						worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
					}
				}
			}
