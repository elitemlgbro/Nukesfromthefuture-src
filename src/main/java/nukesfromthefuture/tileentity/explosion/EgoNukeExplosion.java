...
public class EgoNukeExplosion
{
	...
	public EgoNukeExplosion(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(EntityEgoBlast tsarblast)
	{
		...
		if (n > 0 && n < nlimit)
		{
			...
			{
				repeatCount++;
				if (repeatCount < Nukesfromthefuture.egoNukeSpeed * 2) update(tsarblast);
				else
				{
					repeatCount = 0;
					return true;
		...
		if (dist < radius * radius)
		{
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					if (Y == 0) break;
					Block block = worldObj.getBlock(x + posX, Y, z + posZ);
					if (blockType == 1) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.trololo);
					else if (blockType == 2) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.ego_block);
					else if (blockType == 3) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.trololo);
					else worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.ego_block);
			...
			if (isTree)
			{
				...
				{
					if (Y == 0) break;
					worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.trololo);
					worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
				}
			}
		...
		else if (dist <= radius * radius * 1.3125 * 1.3125)
		{
			...
			if (dist >= radius + 5)
			{
				...
				for (int Y = ylimit; Y >= 0; Y--)
				{
					...
					else
					{
						...
						{
							worldObj.setBlock(x + posX, yy + Yy, z + posZ, Nukesfromthefuture.trololo);
			...
			else
			{
				...
				if (isTree)
				{
					...
					for (int Y = ylimit; Y > ylimit - treeHeight; Y--)
					{
						worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.ego_block);
						worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
					}
				}
			}
