...
public class ExplosionMan {
	...
	public ExplosionMan(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(BoyBlast tsarblast)
	{
		...
		if (n > 0 && n < nlimit)
		{
			...
			{
				repeatCount++;
				if (repeatCount < Nukesfromthefuture.Boyspeed* 2) update(tsarblast);
				else
				{
					repeatCount = 0;
					return true;
