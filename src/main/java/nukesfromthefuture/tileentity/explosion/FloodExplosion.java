...
public class FloodExplosion {
	...
	public FloodExplosion(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(Flood lul)
	{
		...
		if (dist < radius * radius)
		{
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					if (Y == 0) break;
					Block block = worldObj.getBlock(x + posX, Y, z + posZ);
					if (blockType == 1) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.water);
					...
					else worldObj.setBlock(x + posX, Y, z + posZ, Blocks.water);
