...
public class Generic{
	...
	public static void wasteDest(World world, int x, int y, int z) {
		if (!world.isRemote) {
			...
			if (b == Blocks.wooden_door || b == Blocks.iron_door) {
				...
				rand = random.nextInt(10);
				if(rand == 2 || rand == 4){
					world.setBlock(x, y, z, Nukesfromthefuture.Deathinum_ore);
				}
			}
			...
			else if (b.getMaterial() == Material.wood && b.isOpaqueCube() && b != Nukesfromthefuture.waste) {
				world.setBlock(x, y, z, Nukesfromthefuture.waste);
			}
	...
	public static void solinium(World world, int x, int y, int z) {
		if (!world.isRemote) {
			...
			if(b == Blocks.grass || b == Blocks.mycelium || b == Nukesfromthefuture.waste) {
