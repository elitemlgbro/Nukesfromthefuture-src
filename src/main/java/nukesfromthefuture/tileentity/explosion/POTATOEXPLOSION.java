...
public class POTATOEXPLOSION {
	...
	public POTATOEXPLOSION(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(POTATOBLAST potatoblast)
	{
		...
		if (n > 0 && n < nlimit)
		{
			...
			{
				repeatCount++;
				if (repeatCount < Nukesfromthefuture.POTATOSPEED * 2) update(potatoblast);
				else
				{
					repeatCount = 0;
					return true;
		...
		if (dist < radius * radius)
		{
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					if (Y == 0) break;
					Block block = worldObj.getBlock(x + posX, Y, z + posZ);
					if (blockType == 1) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.POTATOblock);
					else if (blockType == 2) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.POTATOblock);
					else if (blockType == 3) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.POTATOblock);
					else worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.POTATOblock);
			...
			if (isTree)
			{
				...
				{
					if (Y == 0) break;
					worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.POTATOblock);
					worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
				}
			}
			...
			if (dist >= radius + 5)
			{
				...
				for (int Y = ylimit; Y >= 0; Y--)
				{
					...
					else
					{
						...
						{
							worldObj.setBlock(x + posX, yy + Yy, z + posZ, Nukesfromthefuture.POTATOblock);
