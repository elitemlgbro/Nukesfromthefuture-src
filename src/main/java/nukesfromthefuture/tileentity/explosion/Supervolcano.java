...
public class Supervolcano {
	...
	public Supervolcano(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(LavaBlast lul)
	{
		...
		if (dist < radius * radius)
		{
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					if (Y == 0) break;
					Block block = worldObj.getBlock(x + posX, Y, z + posZ);
					if (blockType == 1) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.lava);
					else if (blockType == 2) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.lava);
					else if (blockType == 3) worldObj.setBlock(x + posX, Y, z + posZ, Blocks.lava);
					else worldObj.setBlock(x + posX, Y, z + posZ, Blocks.lava);
			...
			if (isTree)
			{
				...
				{
					if (Y == 0) break;
					worldObj.setBlock(x + posX, Y, z + posZ, Blocks.flowing_lava);
					worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
				}
			}
		...
		else if (dist <= radius * radius * 1.3125 * 1.3125)
		{
			...
			if (dist >= radius + 5)
			{
				...
				for (int Y = ylimit; Y >= 0; Y--)
				{
					...
					else
					{
						...
						{
							worldObj.setBlock(x + posX, yy + Yy, z + posZ, Blocks.flowing_lava);
			...
			else
			{
				...
				if (isTree)
				{
					...
					for (int Y = ylimit; Y > ylimit - treeHeight; Y--)
					{
						worldObj.setBlock(x + posX, Y, z + posZ, Blocks.lava);
						worldObj.setBlockMetadataWithNotify(x + posX, Y, z + posZ, metadata, 3);
					}
				}
			}
