...
public class TrololoExplosion {
	...
	public TrololoExplosion(int x, int y, int z, World world, int rad)
	{
		posX = x;
		posY = y;
		posZ = z;
	...
	public boolean update(TrolBlast trolBlast)
	{
		...
		if (n > 0 && n < nlimit)
		{
			...
			{
				repeatCount++;
				if (repeatCount < Nukesfromthefuture.trol_speed * 2) update(trolBlast);
				else
				{
					repeatCount = 0;
					return true;
		...
		if (dist < radius * radius)
		{
			...
			if (dist < limit)
			{
				...
				for (int Y = ylimit; Y > ylimit - (worldObj.rand.nextInt(5) + 2); Y--)
				{
					...
					else if (blockType == 2) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.trololo);
					else if (blockType == 3) worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.trololo);
					else worldObj.setBlock(x + posX, Y, z + posZ, Nukesfromthefuture.trololo);
